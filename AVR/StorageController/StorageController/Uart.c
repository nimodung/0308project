...
ISR(USART_RX_vect) { /* USART Rx Complete */
	static char buf_idx = 0, cmd_idx = 0; //명령어 배열 index 제어 
	RX_data = UDR0;
	
	if(buf_idx < BUFFER_MAX && RX_cmd_count <= COMMAND_MAX) {
		if(RX_data == '\n') { //명령이 들어옴
			buffer[cmd_idx][buf_idx] = 0; //or '\0' //문자열의 마지막에 null 넣어주기
			buf_idx = 0; //다음 명령어를 받기위한 clear
			RX_cmd_count++;
			
			cmd_idx++;
			cmd_idx = cmd_idx % COMMAND_MAX;
		}
		else buffer[cmd_idx][buf_idx++] = RX_data;
