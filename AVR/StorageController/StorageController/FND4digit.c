...
void FND4digit_init(void)
{
	...
	FND_DATA_DDR |= 0b00001111; //포트를 출력(1)으로 설정
	FND_DATA_PORT |= 0b00001111; //LED를 전부 끄는걸로 초기화(VCC에 연결했기때문에 1:꺼짐 0:켜짐)
	return;
}
...
void FND_update_word(char *words)
{
	...
	/*
	FND[0] = FND4digit_font_alphabet[word[3] - 'a'];
	FND[1] = FND4digit_font_alphabet[word[2] - 'a'];
	FND[2] = FND4digit_font_alphabet[word[1] - 'a'];
	FND[3] = FND4digit_font_alphabet[word[0] - 'a'];
	*/
	return;
}

void FND_update_storage_number(char value){
	FND[0] = 255;
	FND[1] = FND4digit_font[value]; 
	FND[2] = 255;
	FND[3] = 255;
}